...
export async function POST(request: Request) {
  try {
    ...
    // Execute the WSL script that will run Nextflow
    const wslCommand = `wsl /home/darfito/run-mednist.sh "${runId}" "${winParamsFilePath}"`;
    ...
    return NextResponse.json({ 
      runId,
      message: 'MedNIST pipeline started in WSL' 
    });
  } catch (error) {
